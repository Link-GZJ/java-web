 ...
 * @version 1.0
 */
public class SendHtmlWithInlineImage {
    final String smtpHost;
    final String username;
    final String password;
    final boolean debug;
    SendHtmlWithInlineImage(String smtpHost, String username, String password, boolean debug){
    ...
    private Session createTLSSession() {
        ...
        Session session = Session.getInstance(props, new Authenticator() {
            protected PasswordAuthentication getPasswordAuthentication() {
                ...
                return new PasswordAuthentication(SendHtmlWithInlineImage.this.username, password);
            }
        });
    ...
    private static void sendEmail() {
        ...
        //获取session 实例
        SendHtmlWithInlineImage se = new SendHtmlWithInlineImage(smtp,username,password,true);
    ...
    private void createTextMessage(String username, Session session) {
        ...
        try {
            ...
            textPart.setContent("<h1>Hello</h1><p><img src=\"cid:img01\"></p><p>这是一封带有内嵌图片的JavaMail邮件</p>","text/html;charset=utf-8");
            multipart.addBodyPart(textPart);
            //添加内嵌image
            BodyPart imagePart = new MimeBodyPart();
            imagePart.setDataHandler(new DataHandler(new ByteArrayDataSource(new FileInputStream("F:\\Download\\芙宁娜.png"),"image/png")));
            imagePart.setHeader("Content-ID","<img01>");
            multipart.addBodyPart(imagePart);
            BodyPart imagePart1 = new MimeBodyPart();
            imagePart1.setFileName("芙宁娜.png");
            imagePart1.setDataHandler(new DataHandler(new ByteArrayDataSource(new FileInputStream("F:\\Download\\芙宁娜.png"),"application/octet-stream")));
